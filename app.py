...
def aggregateData(dataSet, cities):
  areas = []
  for city in cities:
    results = []
    for data in dataSet:
...
def venues():
  ...
  return render_template('pages/venues.html', areas=aggregateData(data, cities))
...
def getShows(venue):
  ...
  upcoming_shows = []
  for s in shows:
    if s.start_time >= datetime.date.today():
...
def show_venue(venue_id):
  ...
  data = Venue.qeury.filter_by(id = venue_id)
  res = []
  for dat in data:
...
def artists():
  ...
  # }, {
  #   "id": 6,
  #   "name": "The Wild Sax Bandsssssssssssssss",
  # }]
...
def search_artists():
  ...
  regexp = '^' + re.escape(request.form.get('search_term', '')) + r'\d+$'
  artists = db.session.query(Artist).filter(Artist.name.regexp(regexp)).all()
  response={
    "count": len(artists),
    "data": artists
  }
...
def show_artist(artist_id):
  ...
  data = Artist.qeury.filter(artist_id)
  res = []
  for dat in data:
    res.append(getArtistShows(dat))
  return render_template('pages/show_artist.html', artist=res[0])
